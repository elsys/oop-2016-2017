...
Interpreter::~Interpreter() {
	...
	for(std::list<Operation*>::reverse_iterator it=operations_.rbegin();
		it!=operations_.rend();++it) {
...
void Interpreter::macro_definition(const std::string& name,
	...
	CompositeOperation* macro=
		new CompositeOperation(name.substr(1,name.size()-1));
